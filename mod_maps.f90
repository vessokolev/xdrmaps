      ...
      private :: bubble_sort
      private :: get_division_per_frame
      private :: get_indx
      ...
      subroutine bubble_sort(array_s,&
                             array_indx,&
                             array)
      !
      ...
      function round_u(argument) result(res)
      !
      ! Function for rounding a floating point number to the closest
      ! integer. This function follows the definition given by equation
      ! E3 in the Supporting Information of DOI:
      ! 10.1021/acs.jpcb.5b06566
      !
      ! Interface variables:
      !
      ...
      if ((argument-floor(argument)).lt.0.5_C_FLOAT) then
         res=floor(argument)
      else
         res=ceiling(argument)
      end if

      end function round_u


      subroutine get_num_intervals_per_frame(box,&
                                             min_step,&
                                             num_intervals,&
                                             corr_step)
      !
      ...
      num_intervals(box_diagonal_indx(2))=&
         round_u(num_intervals(box_diagonal_indx(1))*&
      ...
      subroutine get_division_per_frame(box,&
                                        num_intervals,&
                                        division_step)
      !
      ! The subroutine implements the idea for getting the step for
      ! partitioning each frame into intervals, expressed by equation E5
      ! in the Supporting Information of DOI 10.1021/acs.jpcb.5b06566
      !
      ! Interface variables:
      !
      real(C_FLOAT),dimension(3,3),intent(in) :: box
      integer(C_INT),dimension(3),intent(in) :: num_intervals
      real(C_FLOAT),dimension(3),intent(out) :: division_step
      !
      ! Local variables:
      !
      integer(C_INT) :: i
      ...
      do i=1,3
         division_step(i)=box(i,i)/num_intervals(i)
      end do

      end subroutine get_division_per_frame


      subroutine read_atom_presence(h5_file,&
                                    dsetname,&
                                    presence)
      !
      ...
      integer(HSIZE_T),dimension(:),allocatable :: presence_dims
      integer(HSIZE_T),dimension(:),allocatable :: max_presence_dims
      integer(C_INT) :: error
      integer(C_INT) :: rankr
      integer(C_INT),dimension(:),allocatable :: presence_int
      !
      ...
      ! Open the HDF5 file:
      !
      call h5fopen_f(trim(adjustl(h5_file)),H5F_ACC_RDWR_F,file_id,&
                     error)
      if (error.ne.0) then
         print *
      ...
      subroutine read_atomic_mass(h5_file,&
                                  dsetname,&
                                  mass)
      !
      ...
      call h5sget_simple_extent_dims_f(dataspace,mass_dims,&
                                       max_mass_dims,error)
      ! No need to check the error here!!!
      !
      ! Read the dataset. Before that allocate the dynamic array to
      ...
      call h5dread_f(dset_id,H5T_NATIVE_REAL,mass,mass_dims,&
                     error)
      if (error.ne.0) then
         print *
         print *,'FATAL ERROR: Cannot read the content of the '//&
      ...
      subroutine save_grid_as_hdf5(box_average,&
                                   step_average,&
                                   num_frames,&
                                   num_nodes,&
                                   num_atoms,&
      ...
      integer(HSIZE_T),dimension(1) :: frm_s
      ...
      grid_s=shape(grid)
      !
      ! Open the HDF5 containing the grid array.
      !
      call h5open_f(error)
      !
      ! Create the HDF5 file.
      !
      call h5fcreate_f(trim(adjustl(h5_file)),&
                       H5F_ACC_TRUNC_F,&
                       file_id,&
                       error)
      !
      ...
      call h5dwrite_f(dset_id,&
                      ...
                      error)
      !
      call h5dclose_f(dset_id,&
                      error)
      !
      call h5sclose_f(dspace_id,&
                      error)
      !
      ...
      call h5screate_simple_f(1,&
                              ...
                              error)
      !
      call h5dcreate_f(file_id,'num_frames',&
                       H5T_NATIVE_INTEGER,&
                       dspace_id,&
                       dset_id,&
                       error)
      ...
      call h5screate_simple_f(1,&
                              ...
                              error)
      !
      call h5dcreate_f(file_id,'num_atoms',&
                       H5T_NATIVE_INTEGER,&
                       dspace_id,&
                       dset_id,&
                       error)
      ...
      call h5screate_simple_f(1,&
                              ...
                              error)
      !
      call h5dcreate_f(file_id,'num_nodes',&
                       H5T_NATIVE_INTEGER,&
                       dspace_id,&
                       dset_id,&
                       error)
      ...
      do i=1,3
         ...
         if (coord(i).gt.box(i,i)) then
            corr(i)=coord(i)-box(i,i)
         end if
         if (corr(i).lt.box(i,i)) then
      ...
      subroutine analyze_ave_box_size_trr(trr_file,&
                                          box_average,&
                                          num_atoms,&
                                          num_frames)
      !
      ...
      do while ( trr % STAT == 0 )
         ...
         !
         call integer_to_string(num_frames,&
                                tmp_1_len,&
                                tmp_1)
         if (flag) then
            flag=.false.
      ...
      subroutine analyze_ave_box_size_xtc(xtc_file,&
                                          box_average,&
                                          num_atoms,&
                                          num_frames)
      !
      ...
      subroutine get_mass_grid_trr(trr_file,&
                                   num_atoms,&
                                   num_intervals,&
                                   presence,&
                                   mass,&
      ...
      subroutine get_mass_grid_xtc(xtc_file,&
                                   num_atoms,&
                                   num_intervals,&
                                   presence,&
                                   mass,&
      ...
      subroutine get_vf_grid(trr_file,&
                             num_atoms,&
                             num_intervals,&
                             presence,&
                             v_or_f,&
      ...
      subroutine integer_to_string(int_num,&
                                   str_len,&
                                   str)
      !
