      ...
      character(len=4096) :: presence_h5_file
      character(len=4096) :: mass_h5_file
      character(len=4096) :: output_h5_file
      ...
      !
      call send_input_param_to_main(num_nodes,&
                                    files,&
                                    flag,&
                                    extension)
      !
      ...
      if (extension.eq.'.xtc') then
         call analyze_ave_box_size_xtc(files(1),&
                                       box_average,&
                                       num_atoms,&
                                       num_frames)
      else
         call analyze_ave_box_size_trr(files(1),&
                                       box_average,&
                                       num_atoms,&
                                       num_frames)
      end if
      ...
      !
      if ((num_nodes(1).gt.0).and.&
          (num_nodes(2).gt.0).and.&
          (num_nodes(3).gt.0)) then
         !
         ...
         ! into:
         !
         call get_num_intervals_per_frame(box_average,&
                                          step,&
                                          num_intervals,&
                                          corr_step)
      ...
      !
      call read_atom_presence(files(3),&
                              'presence',&
                              presence)
      !
      ...
      if (flag.eq.'M') then
         ...
         !
         if (extension.eq.'.xtc') then
            call get_mass_grid_xtc(files(1),&
                                   num_atoms,&
                                   num_intervals,&
                                   presence,&
                                   mass,&
            ...
            call get_mass_grid_trr(files(1),&
                                   num_atoms,&
                                   num_intervals,&
                                   presence,&
                                   mass,&
         ...
         !
         code=1
         call save_grid_as_hdf5((/(box_average(i,i),i=1,3)/),&
                                step_average,&
                                num_frames,&
                                num_nodes,&
                                num_atoms,&
