...
!#                      XDRMAPS_D                       #
!#                                                      #
!# TOOL FOR DUMPINT THE COLLECTED GRID STATISTICS AS A  #
!#                  SERIES OF 2D MAPS                   #
!#                                                      #
!########################################################
!
...
logical :: noise_red
!
call check_input(grid_file,sel_dim,slice_width,noise_red,path)
!
...
!
call read_scalar_set(grid_file,'code',code)
!
! Read first the dimension of the grid and freq data sets:
!
call read_int_vector_set(grid_file,'num_nodes',num_nodes)
!
...
! Read the grid array:
!
call read_grid(grid_file,'grid',grid)
!
if (code.gt.1) then
   !
   ! Based on the information given by num_nodes array dynamically
   ...
   !
   call read_freq_grid(grid_file,'freq',freq)
   !
   ...
   !
   call read_scalar_set(grid_file,'num_frames',num_frames)
   !
...
! Read the step array:
!
call read_real_vector_set(grid_file,'step',step)
!
! Define the slice array dimensions:
!
slice_s=get_slice_shape(num_nodes,sel_dim)
!      
allocate(slice(slice_s(1),slice_s(2)))
allocate(slice_filtered(slice_s(1),slice_s(2)))
!
step_2D=get_step_2D(step,sel_dim)
!
if (code.eq.1) then
   !
   do i=1,num_nodes(sel_dim)
      !
      call get_mass_slice(num_nodes,grid,sel_dim,i,slice_width,slice)
      !
      if (noise_red) then
         !
         call stencil(slice_s,slice,stencil_node,w,slice_filtered)
         !
         call write_map_file(path,map_file_prefix,i,slice_s,slice_filtered)
         !
      else
         !
         call write_map_file(path,map_file_prefix,i,slice_s,slice)
         !
      end if
      !
...
if ((code.eq.2).or.(code.eq.3)) then
   ...
   do i=1,num_nodes(sel_dim)
      !
      call get_vf_slice(num_nodes,grid,freq,sel_dim,i,slice_width,slice_s,slice)
      !
      if (noise_red) then
         !
         call stencil(slice_s,slice,stencil_node,w,slice_filtered)
