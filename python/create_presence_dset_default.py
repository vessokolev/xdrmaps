...
#           in the grid statistics. It selects all atoms
#           participating the topology.
...
def select_all_atoms(gro_file):
    # In this particular case only the number of atoms provided in
    # the second line of GRO (Gromos87) file is needed. No need to
    # parse the file after that line!
    ...
    with open(gro_file,'r') as file_obj:
       ...
       for line in file_obj:
          if counter==1:
             ...
             # array will be set 1).
             atom_presence=[1 for i in xrange(numatoms)]
             # Exit the loop here:
             break
          else:
             counter+=1
...
if flag:
   ...
   atom_presence=select_all_atoms(sys.argv[1])
